...
from tts.openai_tts_client import OpenAITTSClient
from tts.piper_tts_client import PiperTTSClient
from tts.mac_tts_client import MacTTSClient
...
class TTSManager:
    ...
    def __init__(self, app, event_bus: EventBus):
        ...
        self.verbose = True
        self.backend_type = ConfigManager.get_value('tts_backend_type', self.app_name)
    ...
    def _get_backend_instance(self):
        if self.backend_type == "openai":
            return OpenAITTSClient(verbose=self.verbose)
        elif self.backend_type == "piper":
            return PiperTTSClient(verbose=self.verbose)
        elif self.backend_type == "mac":
            return MacTTSClient(verbose=self.verbose)
        else:
            raise ValueError("Unsupported TTS backend type")
    ...
    def run_tts(self, text, output_dir=None, split_sentences=True):
        """
        ...
        for current_text in texts_to_process:
            try:
                ...
                result = self.backend.tts(current_text, temp_output_file)
    ...
    def _play_audio(self): 
        ...
        while self.queing or not self.audio_queue.empty():
            ...
            if self.stop_event.is_set() or self.stop_playback:
                break
